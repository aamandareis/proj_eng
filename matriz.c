...
Matriz* novaMatriz(int lin, int col){
	int i;
	Matriz *m = (Matriz*)malloc(sizeof(Matriz*));//aloca na memoria um espa�o para o struct matriz
	if(m == NULL){
	...
	m->colunas = col;
	m->linhas = lin;
	m->matriz = (double**)malloc(lin*sizeof(double*));//aloca na memoria um espa�o as linhas da matriz, feita de ponteiros de double
	if(m->matriz == NULL){
		printf("Erro ao alocar memoria!\n");
		exit(3);
	}
